...
<div class="col-lg-8">
    ...
    <form method="post" action="/dashboard/posts/{{ $post->slug }}" class="mb-4" enctype="multipart/form-data">
      @method('put')
        ...
        <div class="mb-3">
          <label for="qty" class="form-label">Total Harga</label>
          <input type="text" step="any" class="form-control @error('qty') is-invalid @enderror" id="dengan-rupiah" name="qty" required autofocus value="{{ old('qty', $post->qty) }}">
          @error('qty')
          <div class="invalid-feedback">
            {{ $message }}
          </div>
        ...
        <div class="mb-3">
          ...
          <select class="form-select" name="origin_id">
            @foreach ($origins as $origin)
              @if(old('origin_id', $post->origin_id) == $origin->id)
